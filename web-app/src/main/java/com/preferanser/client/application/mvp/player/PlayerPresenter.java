...
public class PlayerPresenter extends Presenter<PlayerPresenter.PlayerView, PlayerPresenter.Proxy>
    ...
    private Optional<String> userIdOptional = Optional.absent();
    private Optional<String> dealIdOptional = Optional.absent();
    ...
    public void prepareFromRequest(PlaceRequest request) {
        ...
        userIdOptional = helper.getStringParameter("user");
        dealIdOptional = helper.getStringParameter("deal");
    }
    ...
    @Override protected void onReveal() {
        ...
        prepositionCards();
        if (gameOptional.isPresent() && dealIdOptional.get().equals(Long.toString(gameOptional.get().getId()))) {
            refreshView();
        } else {
    ...
    @Override public void switchToEditor() {
        ...
        if (dealIdOptional.isPresent()) {
            builder.nameToken(NameTokens.EDITOR).with("deal", dealIdOptional.get());
        } else {
